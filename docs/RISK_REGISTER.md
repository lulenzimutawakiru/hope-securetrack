...
| R-21 | Remaining client-first inventory reads on `inventory/control`, `inventory/reports` and `inventory/valuation` | Phase 17 frontend roadmap: `inventory/control`, `inventory/reports` and `inventory/valuation` migrated to `/api/v2/crud` - `useEntityAll` grid reads (inventory.view gate), report inputs on dedicated TanStack queries (balance/expiry via CRUD `stock_balances` with client-side expiry filter, movement via CRUD `inventory_movements`, reorder/slow/ABC via browser products by design), `inventory_valuations` snapshot writes via `crudCreate` with `entityKeys.entity` invalidation, `inventory_movements` registered (sortable `performed_at`). Closed: no remaining browser-side inventory list reads; product references stay browser-side by design (`products.view` CRUD gate vs page roles) |
| R-22 | Flat ungrouped navigation, no breadcrumbs and no record search across the dashboard | Phase 18 frontend roadmap: grouped sidebar (9 `NAV_GROUPS` with persisted expand/collapse), breadcrumb trail resolving deep feature routes from the `*MENU` catalogs (`src/lib/nav-labels.ts` + `src/components/layout/breadcrumbs.tsx`), branded root 404 (`src/app/not-found.tsx`), and debounced server-side record search in the command palette across warehouses/products/customers/invoices/employees/suppliers/purchase_orders via `/api/v2/crud` (per-entity view-permission gated, failures swallowed); palette trigger visible on mobile |
| R-23 | CSP not enforced on responses; broad inline-script allowance; remaining client-first pages; growing lint warning debt | Middleware `buildCSP()` now applied via `applySecurityHeaders` (strict `default-src 'self'`, `object-src 'none'`, `base-uri 'self'`, `frame-ancestors 'none'`, `worker-src 'self'` for the service worker, `script-src 'self' 'unsafe-inline'` + captcha origin, `script-src-attr 'none'`, `style-src 'self' 'unsafe-inline'`; dev keeps `'unsafe-eval'`). React Query batch: `packaging/lines`, `packaging/materials`, `packaging/rules`, `printers`, `notifications/rules` migrated to `useEntityAll` + `useCrudMutation` (writes auto-invalidate the entity cache, client `company_id` payloads removed - engine derives identity server-side). Lint debt 818 -> 509 warnings / 0 errors (dead code + unused imports removed; set-state-in-effect + exhaustive-deps remain as tracked warnings). Backlog: nonce/CSP hash migration for inline scripts, full ~120-page query-layer sweep, entity catalog -> metadata tables, RLS migration of remaining browser-side reads |
| R-24 | Manual/scripted tenant provisioning - no orchestration, checkpointing, tenant-scoped control plane, or one-time secret handling | Phase 20 provisioning platform: 17-step checkpointed orchestrator (`provisioning/orchestrator.ts`) with compensation/rollback and retry policy; control-plane tables (`tenant_provisioning_jobs`, `provisioning_steps`, `provisioning_job_events`, `provisioning_templates`, `provisioning_api_keys`, `provisioning_key_vault`) RLS-gated to `is_platform_admin()`; platform APIs enforce `platform.admin`/`platform.provision`/`platform.tenants` + MFA-on-privileged + `staffCanAccess("provisioning")`; one-time secrets held in memory only and returned once |
