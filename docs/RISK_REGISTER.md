...
| R-20 | Remaining client-first inventory reads (adjustments, traceability, reservations, transfers, cycle-count/GRN/replenishment list views) plus join-heavy reference resolution | Phase 15 frontend roadmap: `inventory` hub, `stock balances` and `locations` migrated to `/api/v2/crud` - exact server-side KPI totals, `useEntityAll` grid reads with filter-aware cache keys, join-free CRUD reads resolved client-side from reference maps; `stock_reservations`, `stock_transfers`, `inventory_approvals`, `batch_trace_events` registered in the entity registry (inventory.view gate). Known limitation: hub balance aggregate caps at 500 rows (CRUD page-walk cap) - sufficient for KPIs. Join/search-heavy pages (adjustments, traceability, reservations, transfers) deferred to the next phase |
