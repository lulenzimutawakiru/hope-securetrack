...
export async function finAudit(input: {
...
export async function finSoftDelete(table: string, id: string, actorId?: string | null) {
  const { data, error } = await sb()
    .from(table)
...
export async function finRestore(table: string, id: string, actorId?: string | null) {
  const { data, error } = await sb()
    .from(table)
...
export async function finDuplicate(
  ...
  return finCreate(table, { ...rest, ...overrides }, actorId);
}
