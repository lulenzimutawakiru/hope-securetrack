...
class QueryBuilder {
  ...
  private async execute(): Promise<{
    ...
    try {
      ...
      let rows = res.data.data as Record<string, unknown>[];
      // Client-side neq / not_in filters
      for (const f of this.filters) {
        if (f.type === "neq") {
          rows = rows.filter((r) => r[f.col] !== f.val);
        }
        if (f.type === "not_in") {
          rows = rows.filter((r) => !f.val.includes(r[f.col]));
        }
      }
...
export function createClient() {
  return {
    ...
    rpc: async () => ({
      data: null,
      error: { message: "rpc not supported via crud-compat; use API route" },
    }),
    auth: {
    ...
    storage: {
      from: () => {
        throw new Error(
          "Use @/lib/storage/upload for storage; crud-compat does not wrap storage"
        );
      },
    },
  };
