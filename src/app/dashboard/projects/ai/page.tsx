...
import { generatePpmInsights, listPpmInsights, type PpmInsight } from "@/lib/ppm";
import { toast } from "sonner";

export default function PpmAiPage() {
  const { auth } = useUser();
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);
  const [insights, setInsights] = useState<PpmInsight[]>([]);
  ...
  const load = async () => {
    ...
    try {
      const rows = await listPpmInsights(companyId);
      setInsights(
        rows.map((r) => ({
          id: r.id,
  ...
  const run = async () => {
    ...
    setBusy(true);
    try {
      const generated = await generatePpmInsights(companyId);
  ...
  if (loading) return <LoadingState message="Loading AI Project Assistant…" />;

  return (
    <div>
      <PageHeader
        title="AI Project Assistant"
        description="Predict delays · budget overruns · risks · resource recommendations"
        actions={
      ...
      {insights.length === 0 && (
        <p className="text-sm text-muted-foreground mt-4">
          No insights yet. Click <strong>Run analysis</strong> to evaluate schedule, cost, and risk.
        </p>
      )}
    </div>
  );
