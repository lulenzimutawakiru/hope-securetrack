...
import { listCostCenters, createCostCenter } from "@/lib/enterprise-company";
...
export default function CostCentersPage() {
  ...
  const [form, setForm] = useState({ code: "", name: "", manager_name: "" });

  const load = async () => {
    if (!auth?.profile?.company_id) { setLoading(false); return; }
    try {
      setRows((await listCostCenters(auth.profile.company_id)) as Array<Record<string, unknown>>);
  ...
  if (loading) return <LoadingState message="Loading cost centers…" />;

  return (
    <div>
      <PageHeader
        title="Cost Centers"
        description="Budget control · P&L mapping · department allocation"
        actions={<Button size="sm" onClick={() => setOpen(true)}><Plus className="h-4 w-4 mr-1" /> Add</Button>}
      />
      {rows.length === 0 ? (
        <EmptyState title="No cost centers" description="Create cost centers for finance mapping." />
      ) : (
