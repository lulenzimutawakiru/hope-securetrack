...
export const POST = createApiHandler(
  ...
  async ({ req, ctx, params, ip }) => {
    ...
    const id = params.id;
    if (!id) return apiError("VALIDATION", "Transfer id required");
    ...
    try {
      ...
      for (const line of lines ?? []) {
        ...
        const { data: dest, error: destErr } = await destQuery
          .limit(1)
          .maybeSingle();
        ...
        if (dest) {
          const newQty =
            Math.round((Number(dest.quantity_on_hand || 0) + qty) * 10000) /
            10000;
          const totalValue =
            Math.round((Number(dest.total_value || 0) + qty * cost) * 100) / 100;
      ...
      if (updErr || !updated) {
        return apiError(
          "INTERNAL",
          updErr?.message ?? "Failed to receive transfer",
          500
        );
      }
      ...
      await writeServerAudit(admin, {
        ...
        metadata: {
          source: "api/inventory/transfers/[id]/receive",
          received,
        },
        ip_address: ip,
