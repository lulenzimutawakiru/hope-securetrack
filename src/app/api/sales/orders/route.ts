...
export const POST = createApiHandler(
  ...
  async ({ req, ctx, body, ip }) => {
    ...
    try {
      ...
      const price = Math.round(data.unit_price * 100) / 100;
      ...
      for (let attempt = 0; attempt < 2 && !order; attempt++) {
        ...
        const { data: ins, error: insErr } = await admin
          ...
          .insert({
            ...
            requires_production: data.requires_production,
            sales_rep_id: ctx.user.id,
      ...
      if (!creditApproved) {
        ...
        const { error: commErr } = await admin.from("sales_commissions").insert({
          company_id: companyId,
          sales_rep_id: ctx.user.id,
          sales_order_id: order.id,
          basis_amount: total,
