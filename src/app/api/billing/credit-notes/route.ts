...
const schema = z.object({
  ...
  credit_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
...
export async function POST(req: NextRequest) {
  ...
  const rl = await rateLimitStrict(
    `billing-credit-notes:${auth.ctx.user.id}:${ip}`,
    20,
    60_000
  );
  ...
  try {
    ...
    const credit_note_number = await nextBillNumberServer(admin, companyId, "CRN");
    ...
    const { data: note, error: noteErr } = await admin
      ...
      .insert({
        ...
        credit_date: parsed.data.credit_date ?? new Date().toISOString().slice(0, 10),
        reason_code: parsed.data.reason_code || "adjustment",
        ...
        lines_json: [
          {
            description: parsed.data.reason || "Credit",
            quantity: 1,
            unit_price: sub,
            tax_amount: tax,
          },
      ...
      .single();
    if (noteErr || !note) {
      return apiError("INTERNAL", noteErr?.message ?? "Failed to issue credit note", 500);
    }
