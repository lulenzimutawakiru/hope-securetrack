...
/** Local wall-clock (Africa/Kampala) hour + minute used for late rules. */
...
/**
 * Web clock in/out (attendance money path).
 *
 * company_id is always derived from the authenticated session; the employee
 * must belong to the caller's company. Late / productive / overtime minutes
 * and the labor cost snapshot are computed server-side. The labor cost entry
 * is written with the employee's stored hourly rate (default 15000 UGX) and
 * OT at 1.5x, mirroring the legacy page behaviour but server-side.
 */
export async function POST(req: NextRequest) {
  ...
  const rl = await rateLimitStrict(
    `workforce-attendance:${auth.ctx.user.id}:${ip}`,
    30,
    60_000
  );
  ...
  try {
    ...
    if (parsed.data.action === "in") {
      ...
      if (existing) {
        const { data: upd, error: updErr } = await admin
          ...
          .single();
        if (updErr || !upd) {
          return apiError("INTERNAL", updErr?.message ?? "Clock in failed", 500);
        }
        recordId = upd.id;
      } else {
        const { data: ins, error: insErr } = await admin
          ...
          .single();
        if (insErr || !ins) {
          return apiError("INTERNAL", insErr?.message ?? "Clock in failed", 500);
        }
        recordId = ins.id;
      }
    ...
    const workedMin = Math.max(
      0,
      Math.round((now.getTime() - new Date(existing.check_in).getTime()) / 60000)
    );
    ...
    // Labor cost snapshot (server-side, money path)
    ...
    if (costErr) {
      // Revert the check-out so the punch is not half-recorded
      await admin
        .from("attendance_records")
        .update({
          check_out: null,
