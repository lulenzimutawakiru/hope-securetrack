...
/** Count-based PO number, mirroring the page's PO-YYYY-###### pattern. */
 ...
 *
 * Supplier and products must belong to the caller's company; totals are
 * computed server-side; company_id comes from the session. The PO is created
 * with status "approved" to preserve the page's existing behavior.
 */
export async function POST(req: NextRequest) {
  ...
  const rl = await rateLimitStrict(
    `procurement-orders:${auth.ctx.user.id}:${ip}`,
    20,
    60_000
  );
  ...
  try {
    ...
    if (!supplier) return apiError("NOT_FOUND", "Supplier not found in this company", 404);

    const productIds = [...new Set(parsed.data.lines.map((l) => l.product_id))];
    ...
    const lineRows = parsed.data.lines.map((l, i) => {
      ...
      const price = Math.round(((l.unit_price ?? Number(product.standard_cost)) || 0) * 100) / 100;
    ...
    const { data: po, error: poErr } = await admin
      ...
      .insert({
        ...
        total_amount: total,
        status: "approved",
        po_type: parsed.data.po_type || "standard",
        payment_terms: parsed.data.payment_terms || "Net 30",
      ...
      .single();
    if (poErr || !po) {
      return apiError("INTERNAL", poErr?.message ?? "Failed to create purchase order", 500);
    }
    ...
    await writeServerAudit(admin, {
      ...
      after_state: { supplier_id: po.supplier_id, lines: lineRows.length, total },
