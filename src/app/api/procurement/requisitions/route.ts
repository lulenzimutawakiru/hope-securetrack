...
const schema = z.object({
  ...
  required_by: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().nullable(),
...
/** Count-based requisition number, mirroring the page's PR-YYYY-###### pattern. */
 ...
 *
 * Estimated totals are computed server-side from the product's standard cost;
...
export async function POST(req: NextRequest) {
  ...
  const rl = await rateLimitStrict(
    `procurement-requisitions:${auth.ctx.user.id}:${ip}`,
    20,
    60_000
  );
  ...
  try {
    ...
    const { data: pr, error: prErr } = await admin
      ...
      .insert({
        ...
        quantity: qty,
        uom: parsed.data.uom || "EA",
        suggested_supplier: parsed.data.suggested_supplier ?? null,
        estimated_unit_cost: unitCost,
        estimated_total: estimatedTotal,
        department: parsed.data.department || "Production",
        request_type: parsed.data.request_type || "material",
        priority: parsed.data.priority || "medium",
        justification: parsed.data.justification || null,
        reason: parsed.data.justification || "Procurement request",
      ...
      .single();
    if (prErr || !pr) {
      return apiError("INTERNAL", prErr?.message ?? "Failed to create requisition", 500);
    }
    ...
    await writeServerAudit(admin, {
      ...
      after_state: { product_id: pr.product_id, quantity: qty, estimated_total: estimatedTotal },
