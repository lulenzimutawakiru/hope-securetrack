...
import {
  ppmList, ppmCreate, ppmUpdate, ppmSoftDelete, ppmDuplicate,
  ppmBulkStatus, ppmNextNumber, toCsv, downloadCsv,
} from "@/lib/ppm/crud";
import { toast } from "sonner";

export type PpmFieldDef = {
  key: string;
  label: string;
...
export function PpmEntityPage({ config }: { config: PpmEntityConfig }) {
  ...
  const load = async () => {
    ...
    try {
      const data = await ppmList(config.table, {
        companyId,
  ...
  const openCreate = async () => {
    ...
    for (const f of config.fields) {
      if (f.autoNumber && companyId) {
        init[f.key] = await ppmNextNumber(
          config.table,
          companyId,
          f.autoNumber,
          f.key
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editId) {
        await ppmUpdate(config.table, editId, payload, auth.user.id);
        toast.success("Updated");
      } else {
        await ppmCreate(config.table, payload, auth.user.id);
  ...
  const remove = async (id: string) => {
    ...
    try {
      await ppmSoftDelete(config.table, id, auth?.user.id);
  ...
  const duplicate = async (row: Record<string, unknown>) => {
    try {
      ...
      if (config.numberField && config.numberPrefix && companyId) {
        overrides[config.numberField] = await ppmNextNumber(
          config.table,
          companyId,
          config.numberPrefix,
          config.numberField
      ...
      await ppmDuplicate(config.table, row.id as string, overrides, auth?.user.id);
  ...
  const bulkStatus = async (st: string) => {
    ...
    try {
      await ppmBulkStatus(config.table, Array.from(selected), st, auth?.user.id);
