...
import {
  payList, payCreate, payUpdate, paySoftDelete, payDuplicate,
  payBulkStatus, payNextNumber, toCsv, downloadCsv, payRestore, payImportCsv,
} from "@/lib/payroll/crud";
import { toast } from "sonner";

export type PayFieldDef = {
  key: string;
  label: string;
...
export function PayEntityPage({ config }: { config: PayEntityConfig }) {
  ...
  const load = async () => {
    ...
    try {
      const data = await payList(config.table, {
        companyId,
  ...
  const openCreate = async () => {
    ...
    for (const f of config.fields) {
      if (f.autoNumber && companyId) {
        init[f.key] = await payNextNumber(
          config.table,
          companyId,
          f.autoNumber,
          f.key
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editId) {
        await payUpdate(config.table, editId, payload, auth.user.id);
        toast.success("Updated");
      } else {
        await payCreate(config.table, payload, auth.user.id);
  ...
  const remove = async (id: string) => {
    ...
    try {
      await paySoftDelete(config.table, id, auth?.user.id);
  ...
  const restore = async (id: string) => {
    try {
      await payRestore(config.table, id, auth?.user.id);
  ...
  const duplicate = async (row: Record<string, unknown>) => {
    try {
      ...
      if (config.numberField && config.numberPrefix && companyId) {
        overrides[config.numberField] = await payNextNumber(
          config.table,
          companyId,
          config.numberPrefix,
          config.numberField
      ...
      await payDuplicate(config.table, row.id as string, overrides, auth?.user.id);
  ...
  const bulkStatus = async (st: string) => {
    ...
    try {
      await payBulkStatus(config.table, Array.from(selected), st, auth?.user.id);
  ...
  const onImportFile = async (file: File) => {
    ...
    setBusy(true);
    try {
      const result = await payImportCsv(config.table, text, {
        companyId,
      ...
      if (result.failed) {
        toast.error(`${result.failed} row(s) failed validation/insert`);
      }
      await load();
    } catch (e) {
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        <EmptyState title="No records" description="Create the first record or import a CSV." />
      ) : (
