...
import {
  salesList, salesCreate, salesUpdate, salesSoftDelete, salesDuplicate,
  salesBulkStatus, salesNextNumber, toCsv, downloadCsv, salesRestore,
} from "@/lib/sales/crud";
...
export function SalesEntityPage({ config }: { config: SalesEntityConfig }) {
  ...
  const load = async () => {
    ...
    try {
      const data = await salesList(config.table, {
        companyId,
  ...
  const openCreate = async () => {
    ...
    for (const f of config.fields) {
      if (f.autoNumber && companyId) {
        init[f.key] = await salesNextNumber(
          config.table,
          companyId,
          f.autoNumber,
          f.key
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editId) {
        await salesUpdate(config.table, editId, payload, auth.user.id);
        toast.success("Updated");
      } else {
        await salesCreate(config.table, payload, auth.user.id);
  ...
  const remove = async (id: string) => {
    ...
    try {
      await salesSoftDelete(config.table, id, auth?.user.id);
  ...
  const restore = async (id: string) => {
    try {
      await salesRestore(config.table, id, auth?.user.id);
  ...
  const duplicate = async (row: Record<string, unknown>) => {
    try {
      ...
      if (config.numberField && config.numberPrefix && companyId) {
        overrides[config.numberField] = await salesNextNumber(
          config.table,
          companyId,
          config.numberPrefix,
          config.numberField
      ...
      await salesDuplicate(config.table, row.id as string, overrides, auth?.user.id);
  ...
  const bulkStatus = async (st: string) => {
    ...
    try {
      await salesBulkStatus(config.table, Array.from(selected), st, auth?.user.id);
      toast.success(`Status â†’ ${st}`);
      setSelected(new Set());
      await load();
    } catch (e) {
  ...
  if (loading) return <LoadingState message={`Loading ${config.title}â€¦`} />;
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <div className="relative flex-1 min-w-[180px] max-w-sm">
          ...
          <Input className="pl-8" placeholder="Searchâ€¦" value={q} onChange={(e) => setQ(e.target.value)} />
        </div>
        ...
        {selected.size > 0 && config.statusOptions && (
          <Select onValueChange={bulkStatus}>
            ...
            <SelectContent>
              {config.statusOptions.map((s) => (
                <SelectItem key={s} value={s}>â†’ {s}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        )}
                  ...
                  {config.columns.map((c) => (
                    <TableCell key={c.key} className="text-sm">
                      {c.key === (config.statusField || "status") ? (
                        <Badge variant="outline">{String(r[c.key] ?? "â€”")}</Badge>
                      ) : (
                        String(r[c.key] ?? "â€”")
                      )}
                    </TableCell>
