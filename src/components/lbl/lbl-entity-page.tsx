...
import {
  lblList, lblCreate, lblUpdate, lblSoftDelete, lblDuplicate,
  lblBulkStatus, lblNextNumber, toCsv, downloadCsv, lblRestore,
} from "@/lib/lbl/crud";
import { toast } from "sonner";

export type LblFieldDef = {
  key: string;
  label: string;
...
export function LblEntityPage({ config }: { config: LblEntityConfig }) {
  ...
  const load = async () => {
    ...
    try {
      const data = await lblList(config.table, {
        companyId,
  ...
  const openCreate = async () => {
    ...
    for (const f of config.fields) {
      if (f.autoNumber && companyId) {
        init[f.key] = await lblNextNumber(
          config.table,
          companyId,
          f.autoNumber,
          f.key
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editId) {
        await lblUpdate(config.table, editId, payload, auth.user.id);
        toast.success("Updated");
      } else {
        await lblCreate(config.table, payload, auth.user.id);
  ...
  const remove = async (id: string) => {
    ...
    try {
      await lblSoftDelete(config.table, id, auth?.user.id);
  ...
  const restore = async (id: string) => {
    try {
      await lblRestore(config.table, id, auth?.user.id);
  ...
  const duplicate = async (row: Record<string, unknown>) => {
    try {
      ...
      if (config.numberField && config.numberPrefix && companyId) {
        overrides[config.numberField] = await lblNextNumber(
          config.table,
          companyId,
          config.numberPrefix,
          config.numberField
      ...
      await lblDuplicate(config.table, row.id as string, overrides, auth?.user.id);
  ...
  const bulkStatus = async (st: string) => {
    ...
    try {
      await lblBulkStatus(config.table, Array.from(selected), st, auth?.user.id);
      toast.success(`Status Ã¢â€ â€™ ${st}`);
      setSelected(new Set());
      await load();
    } catch (e) {
  ...
  if (loading) return <LoadingState message={`Loading ${config.title}Ã¢â‚¬Â¦`} />;
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <div className="relative flex-1 min-w-[180px] max-w-sm">
          ...
          <Input className="pl-8" placeholder="SearchÃ¢â‚¬Â¦" value={q} onChange={(e) => setQ(e.target.value)} />
        </div>
        ...
        {selected.size > 0 && config.statusOptions && (
          <Select onValueChange={bulkStatus}>
            ...
            <SelectContent>
              {config.statusOptions.map((s) => (
                <SelectItem key={s} value={s}>Ã¢â€ â€™ {s}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        )}
                  ...
                  {config.columns.map((c) => (
                    <TableCell key={c.key} className="text-sm">
                      {c.key === (config.statusField || "status") ? (
                        <Badge variant="outline">{String(r[c.key] ?? "Ã¢â‚¬â€")}</Badge>
                      ) : (
                        String(r[c.key] ?? "Ã¢â‚¬â€")
                      )}
                    </TableCell>
                  ))}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button onClick={save} disabled={busy}>{busy ? "SavingÃ¢â‚¬Â¦" : "Save"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
