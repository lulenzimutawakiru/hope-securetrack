...
import {
  fleetList, fleetCreate, fleetUpdate, fleetSoftDelete, fleetDuplicate,
  fleetBulkStatus, fleetNextNumber, toCsv, downloadCsv, fleetRestore,
} from "@/lib/fleet/crud";
import { toast } from "sonner";

export type FleetFieldDef = {
  key: string;
  label: string;
...
export function FleetEntityPage({ config }: { config: FleetEntityConfig }) {
  ...
  const load = async () => {
    ...
    try {
      const data = await fleetList(config.table, {
        companyId,
  ...
  const openCreate = async () => {
    ...
    for (const f of config.fields) {
      if (f.autoNumber && companyId) {
        init[f.key] = await fleetNextNumber(
          config.table,
          companyId,
          f.autoNumber,
          f.key
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editId) {
        await fleetUpdate(config.table, editId, payload, auth.user.id);
        toast.success("Updated");
      } else {
        await fleetCreate(config.table, payload, auth.user.id);
  ...
  const remove = async (id: string) => {
    ...
    try {
      await fleetSoftDelete(config.table, id, auth?.user.id);
  ...
  const restore = async (id: string) => {
    try {
      await fleetRestore(config.table, id, auth?.user.id);
  ...
  const duplicate = async (row: Record<string, unknown>) => {
    try {
      ...
      if (config.numberField && config.numberPrefix && companyId) {
        overrides[config.numberField] = await fleetNextNumber(
          config.table,
          companyId,
          config.numberPrefix,
          config.numberField
      ...
      await fleetDuplicate(config.table, row.id as string, overrides, auth?.user.id);
  ...
  const bulkStatus = async (st: string) => {
    ...
    try {
      await fleetBulkStatus(config.table, Array.from(selected), st, auth?.user.id);
