...
import {
  attList, attCreate, attUpdate, attSoftDelete, attDuplicate,
  attBulkStatus, attNextNumber, toCsv, downloadCsv,
} from "@/lib/attendance/crud";
import { toast } from "sonner";

export type AttFieldDef = {
  key: string;
  label: string;
...
export function AttEntityPage({ config }: { config: AttEntityConfig }) {
  ...
  const load = async () => {
    ...
    try {
      const data = await attList(config.table, {
        companyId,
  ...
  const openCreate = async () => {
    ...
    for (const f of config.fields) {
      if (f.autoNumber && companyId) {
        init[f.key] = await attNextNumber(
          config.table,
          companyId,
          f.autoNumber,
          f.key
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editId) {
        await attUpdate(config.table, editId, payload, auth.user.id);
        toast.success("Updated");
      } else {
        await attCreate(config.table, payload, auth.user.id);
  ...
  const remove = async (id: string) => {
    ...
    try {
      await attSoftDelete(config.table, id, auth?.user.id);
  ...
  const duplicate = async (row: Record<string, unknown>) => {
    try {
      ...
      if (config.numberField && config.numberPrefix && companyId) {
        overrides[config.numberField] = await attNextNumber(
          config.table,
          companyId,
          config.numberPrefix,
          config.numberField
      ...
      await attDuplicate(config.table, row.id as string, overrides, auth?.user.id);
  ...
  const bulkStatus = async (st: string) => {
    ...
    try {
      await attBulkStatus(config.table, Array.from(selected), st, auth?.user.id);
