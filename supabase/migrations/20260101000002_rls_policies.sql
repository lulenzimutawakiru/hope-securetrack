...
CREATE OR REPLACE FUNCTION auth.has_permission(p_permission VARCHAR)
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    SELECT 1
...
CREATE OR REPLACE FUNCTION auth.has_any_permission(p_permissions VARCHAR[])
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    SELECT 1
    FROM user_profiles up
...
CREATE POLICY companies_select ON companies FOR SELECT
  USING (id = auth.user_company_id() OR auth.is_super_admin());

CREATE POLICY companies_update ON companies FOR UPDATE
  USING (id = auth.user_company_id() AND auth.has_permission('settings.manage'));
...
CREATE POLICY user_profiles_update ON user_profiles FOR UPDATE
  USING (
    id = auth.uid()
    OR (company_id = auth.user_company_id() AND auth.has_permission('users.manage'))
  );

CREATE POLICY user_profiles_insert ON user_profiles FOR INSERT
  WITH CHECK (
    company_id = auth.user_company_id() AND auth.has_permission('users.manage')
  );
...
CREATE POLICY warehouse_racks_all ON warehouse_racks FOR ALL
  USING (
    warehouse_id IN (SELECT id FROM warehouses WHERE company_id = auth.user_company_id())
  );
...
CREATE POLICY production_machines_all ON production_machines FOR ALL
  USING (
    factory_id IN (SELECT id FROM factories WHERE company_id = auth.user_company_id())
  );
...
CREATE POLICY batches_insert ON production_batches FOR INSERT
  WITH CHECK (
    ...
    AND auth.has_any_permission(ARRAY['production.create', 'production.manage'])
  );

CREATE POLICY batches_update ON production_batches FOR UPDATE
  USING (
    company_id = auth.user_company_id()
    AND auth.has_any_permission(ARRAY['production.edit', 'production.manage', 'quality.approve'])
  );
...
CREATE POLICY qr_codes_insert ON qr_codes FOR INSERT
  WITH CHECK (
    company_id = auth.user_company_id()
    AND auth.has_any_permission(ARRAY['qr.generate', 'production.manage'])
  );

CREATE POLICY qr_codes_update ON qr_codes FOR UPDATE
  USING (
    company_id = auth.user_company_id()
    AND auth.has_any_permission(ARRAY['qr.generate', 'printing.reprint', 'production.manage'])
  );
...
CREATE POLICY print_jobs_insert ON print_jobs FOR INSERT
  WITH CHECK (
    company_id = auth.user_company_id()
    AND auth.has_any_permission(ARRAY['printing.create', 'printing.manage'])
  );

CREATE POLICY print_jobs_update ON print_jobs FOR UPDATE
  USING (
    company_id = auth.user_company_id()
    AND auth.has_any_permission(ARRAY['printing.manage', 'printing.reprint'])
  );
...
CREATE POLICY inventory_movements_insert ON inventory_movements FOR INSERT
  WITH CHECK (
    company_id = auth.user_company_id()
    AND auth.has_any_permission(ARRAY['inventory.move', 'inventory.manage'])
  );
...
CREATE POLICY counterfeit_reports_update ON counterfeit_reports FOR UPDATE
  USING (
    company_id = auth.user_company_id()
    AND auth.has_any_permission(ARRAY['fraud.manage', 'fraud.investigate'])
  );
...
CREATE POLICY audit_logs_select ON audit_logs FOR SELECT
  USING (
    company_id = auth.user_company_id()
    AND auth.has_any_permission(ARRAY['audit.view', 'audit.manage'])
  );
...
CREATE POLICY roles_select ON roles FOR SELECT
  USING (company_id = auth.user_company_id() OR company_id IS NULL);
...
CREATE POLICY role_permissions_select ON role_permissions FOR SELECT
  USING (
    role_id IN (SELECT id FROM roles WHERE company_id = auth.user_company_id() OR company_id IS NULL)
  );
...
CREATE POLICY system_settings_update ON system_settings FOR UPDATE
  USING (
    company_id = auth.user_company_id() AND auth.has_permission('settings.manage')
  );
